...
class Boid {
  //Constructor
    ...
    */
  constructor(parent, bType, story, id) {
    //Characteristics
  ...
  */
  ...
  alignment() {
    ...
    for (let other of boids) {
      if((activeStory != null && activeStory.isPlaying() && other.bType === this.bType) ||
         (activeStory == null || (activeStory != null && !activeStory.isPlaying())))
      {
  ...
  render(){
    ...
    push();           //push the current drawing matrix to the stack (start at 0,0)
      ...
      //Draw the boids triangle shape (top, bottom left, bottom right)
      triangle(-boidSize*2, -boidSize,
               -boidSize*2, boidSize,
               boidSize*2, 0);
